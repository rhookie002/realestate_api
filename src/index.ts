  ...
  try {
    ...
    if (street && N8N_WEBHOOK_URL) {
      try {
        ...
        propertyDetail = await n8nResponse.json();
        console.log('[address-search] PropertyDetail from n8n:', !!propertyDetail);
      } catch (err) {
        console.error('[address-search] n8n webhook failed:', err);
      }
    }
    ...
    if (propertyDetail && data && data.data) {
      ...
      const isDuplicate = data.data.some((item: any) => 
        String(item.id || item.propertyId || '') === detailId
      );
      ...
      if (!isDuplicate) {
        ...
        console.log('[address-search] PropertyDetail inserted at top');
      }
    }
